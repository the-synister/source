...
{
    AudioPlayHead::CurrentPositionInfo pos = positionInfo[getAudioIndex()];

    // get GUI params
    int midiSeq[] = {SynthParams::SeqNote1.get(), // TODO: später schleife parse bei init objekte mit notenwert und hold wert und evt. startzeit
                     SynthParams::SeqNote2.get(),
                     SynthParams::SeqNote3.get(),
                     SynthParams::SeqNote4.get()};
    float seqSpeed = SynthParams::seqSpeed.get(); // 1.0 entspricht 1/4 Note
    double seqNoteLength = jmin(SynthParams::seqNoteLength.get(), seqSpeed);

    double currPos = pos.ppqPosition;

    // start sequencer if host is playing
    if (pos.isPlaying)
    {
        if(!seqIsPlaying)
        {
            if(currPos >= seqTimeCounter)
            {
                seqCounter++;
                seqTimeCounter += seqSpeed;

                if (seqCounter > 3) // repeat midiSeq TODO: modulo
                {
                    seqCounter = 0;
                }

                // play valid midi note
                if (midiSeq[seqCounter] != -1)
                {
                    MidiMessage m = MidiMessage::noteOn(1, midiSeq[seqCounter], 0.8f); // timestamp?
                    midiMessages.addEvent(m, 0); // TODO: was macht sampleNumber???
                    seqIsPlaying = true;
                }
            }
        }
        ...
        {
            // TODO: if midi changed, while holdNote, then needs noteoff message

            // noteOff
            if(currPos >= seqTimeCounter - seqSpeed + seqNoteLength)
            {
                if (midiSeq[seqCounter] != -1)
                {
                    MidiMessage m = MidiMessage::noteOff(1, midiSeq[seqCounter]);
    ...
    {
        // stop sequencer
        // klappt aber evtl problem wenn midi keyboard gerade holdNote, weil dann abbruch
        // TODO: unterscheiden zwischen stop und pause
        if (seqCounter != -1) // nur 1 mal clear all notes 
        {
            seqIsPlaying = false;
            //seqTimeCounter = trunc(fmod(pos.ppqPosition, seqSpeed)); // nicht ganz, da er erst mal alle vorigen seq durchgeht
            seqTimeCounter = 0.0;
            seqCounter = -1;
